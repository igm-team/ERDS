...
if($chr=~m/^c*h*r*X|c*h*r*Y|c*h*r*\d+/){
	...
	print "[$time] Starting $ERDS_dir/read_bam_new script with parameters '$bam_file_to_read' $ins_pem $min_det_size '$chr' $aligned_length_thh $CIAGR_length $large_pem $win_size_del $read_length '$chr_rc' '$chr_sd_rc' '$chr_pem_sam'\n";
	system("$ERDS_dir/read_bam_new '$bam_file_to_read' $ins_pem $min_det_size '$chr' $aligned_length_thh $CIAGR_length $large_pem $win_size_del $read_length '$chr_rc' '$chr_sd_rc' '$chr_pem_sam' > '$sample_dir/debug/read_bam_chr$chr.txt' 2> '$sample_dir/debug/read_bam_chr$chr.err'");
