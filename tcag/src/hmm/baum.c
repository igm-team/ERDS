	...
	double **gamma, int *pniter, double *plogprobinit, double *plogprobfinal)
{
	...
	ForwardWithScale(phmm, T, O, L, alpha, scale, &logprobf);
	*plogprobinit = logprobf; /* log P(O |intial model) */
	BackwardWithScale(phmm, T, O, L, beta, scale, &logprobb);

	ComputeGamma(phmm, T, alpha, beta, gamma);
	ComputeXi(phmm, T, O, L, alpha, beta, xi);
	logprobprev = logprobf;

	do  {	
		for (i = 1; i <= phmm->N; i++) {
			phmm->pi[i] = gamma[1][i]*0.999996+0.000001;
		}
