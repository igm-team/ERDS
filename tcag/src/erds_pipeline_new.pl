...
print "\n############################$erds_version############################\n";
my $path = abs_path($0);
if($path=~/([\S\/]*)\/erds_pipeline_new\.pl$/){
	$ERDS_dir=$1;
}
...
die "Cannot find bam_file $bam_file\n" if ($bam_file ne "ERDS_SEPARATE_BAMS" && !-e $bam_file);
...
system("mkdir -p $sample_dir/debug") if(!-e "$sample_dir/debug");
...
while(my $in=<FAI>){
	...
	if ($bam_file eq "ERDS_SEPARATE_BAMS" && !-e "$1.bam") {
		warn "WARNING: chromosome $1 found in $ref_fai_file, but no corresponding BAM file found.\n";
		next;
	}
...
#####single CPU
	...
	system("perl $script_dir/rd_bam_single_new.pl $parameter_sample $functions $num_processors") if($begin_step<=0 and $end_step>=0);
	...
	system("perl $script_dir/single.pl hmm.pl $parameter_sample $functions $num_processors") if($begin_step<=3 and $end_step>=3);
	printtime("ERDS finished step 3 in $samplename...");
	
	#events, step 4
	printtime("ERDS stared step 4 in $samplename...");
	system("perl $script_dir/single.pl events.pl $parameter_sample $functions $num_processors") if($begin_step<=4 and $end_step>=4);
	...
	system("perl $script_dir/single.pl hmm_dup.pl $parameter_sample $functions $num_processors") if($begin_step<=7 and $end_step>=7);
	printtime("ERDS finished step 7 in $samplename...");
	
	#events_dup, step 8
	printtime("ERDS stared step 8 in $samplename...");
	system("perl $script_dir/single.pl events_dup.pl $parameter_sample $functions $num_processors") if($begin_step<=8 and $end_step>=8);
...
#functions
sub get_samplename{
	if ($bam_file eq "ERDS_SEPARATE_BAMS") {
		my $first_chr = `head -n 1 $ref_fai_file | cut -f 1 | tr -d "\n\r"`; # Get the first chromosome listed in the FASTA index file
		open(ST, "$samtools view $SAMTOOLS_EXTRA_FLAGS -H $first_chr.bam|");
	} else {
		open(ST, "$samtools view $SAMTOOLS_EXTRA_FLAGS -H $bam_file|");
	}
	while(my $in=<ST>) {
		if($in=~/\sSM:(\S+)\s/){
			$samplename=$1;
...
sub test_bam{
	...
	for(my $j=0; $j<=$num_ram; $j++){
		...
		my $bam_file_to_read = $bam_file eq "ERDS_SEPARATE_BAMS" ? "$chr_ran.bam" : $bam_file;
		open(ST, "$samtools view $SAMTOOLS_EXTRA_FLAGS -q 10 $bam_file_to_read $chr_ran:$start_ran-$end_ran| awk '{print \$2 \"\t\" \$5 \"\t\" \$6 \"\t\" \$9 \"\t\" length(\$10) \"\t\" \$11'} |");
